...
import FSS.School;
import FSSFFA.FfaSchool;
import FSSFFASAR.FfaSarSchool;
import FSSNF.NfSchool;
import FSSNFSAR.NfSarSchool;
import FSSSAR.SarSchool;
import rFSS.RSchool;
import wFSS.WSchool;
import wFSSNF.NfWSchool;
import wFSSNFSAR.NfSarWSchool;
import wFSSSAR.WSarSchool;
import wrFSS.WRSchool;
import wrFSSnf.WRNFSchool;
import wrFSSnfe.WRNFESchool;
import wrFSSnfg.WRNFGSchool;
import wrFSSnfp.WRNFPSchool;
...
public class Run {
	...
	//11_28 -> C01_C18
	//29 = Test
	public static int FITNESS_FUNCTION = 3;
	...
	public static void main(String[] args) throws IOException{
		...
		case 2:
			...
			algorithmName = "FSS-SAR";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				SarSchool schoolToRun = new SarSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
		...
		case 3:
			...
			algorithmName = "FSS-NF";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				NfSchool schoolToRun = new NfSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
		...
		case 4:
			...
			algorithmName = "FSS-NFSAR";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				NfSarSchool schoolToRun = new NfSarSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
		...
		case 5:
			...
			algorithmName = "FSS-FFA";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				FfaSchool schoolToRun = new FfaSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
		...
		case 6:
			...
			algorithmName = "FSS-FFASAR";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				FfaSarSchool schoolToRun = new FfaSarSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
		...
		case 7:
			...
			double[] feasibility = new double[3];
			double epsilon = 0;
			double epsilonZero = 0;
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				RSchool schoolToRun = new RSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
				feasibility = schoolToRun.getBestFeasibility();
				meanFeasibility+=feasibility[1];
				epsilon = schoolToRun.getEpsilon();
				epsilonZero = schoolToRun.getEpsilonZero();
				
				convergenceDataPerSimulation.add(schoolToRun.getBestFitnessConvergence());
				feasibilityConvergenceDataPerSimulation.add(schoolToRun.getBestFeasibilityConvergence());
			...
			//System.out.println("Epsilon Zero:");
			//System.out.println(epsilonZero);
			//System.out.println("Epsilon:");
			//System.out.println(epsilon);
			//System.out.println("Feasibility:");
			//printArray(feasibility);
			
			if(outputFileOnOff){
				saveFitnessConvergenceForAllSimulationsFile();
				saveFeasibilityConvergenceForAllSimulationsFile();
			}
			break;			
		case 8:
		...
		case 9:
			...
			algorithmName = "wFSS-SAR";
			
			for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
				WSarSchool schoolToRun = new WSarSchool();
				schoolToRun.runSearch();
				meanResult += schoolToRun.getBestFitness();
				lastBestPosition = schoolToRun.getBestPosition();
			...
			case 10:
				...
				algorithmName = "wFSS-NF";
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					NfWSchool schoolToRun = new NfWSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
			...
			case 11:
				...
				algorithmName = "wFSS-NFSAR";
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					NfSarWSchool schoolToRun = new NfSarWSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
			...
			case 12:
				...
				double[] feasibilitywr = new double[3];
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					WRSchool schoolToRun = new WRSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
					feasibilitywr = schoolToRun.getBestFeasibility();
			...
			case 13:
				...
				algorithmName = "wrFSSnf";
				
				double[] feasibilitywrnf = new double[3];
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					WRNFSchool schoolToRun = new WRNFSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
					feasibilitywrnf = schoolToRun.getBestFeasibility();
					meanFeasibility+=feasibilitywrnf[1];
			...
			case 14:
				...
				algorithmName = "wrFSSnfe";
				
				double[] feasibilitywrnfe = new double[3];
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					WRNFESchool schoolToRun = new WRNFESchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
					feasibilitywrnfe = schoolToRun.getBestFeasibility();
					meanFeasibility+=feasibilitywrnfe[1];
			...
			case 15:
				...
				algorithmName = "wrFSSnfg";
				
				double[] feasibilitywrnfg = new double[3];
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					WRNFGSchool schoolToRun = new WRNFGSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
					feasibilitywrnfg = schoolToRun.getBestFeasibility();
					meanFeasibility+=feasibilitywrnfg[1];
			...
			case 16:
				...
				algorithmName = "wrFSSnfp";
				
				double[] feasibilitywrnfp = new double[3];
				
				for(int i=0;i<NUMBER_OF_SIMULATIONS;i++){
					WRNFPSchool schoolToRun = new WRNFPSchool();
					schoolToRun.runSearch();
					meanResult += schoolToRun.getBestFitness();
					lastBestPosition = schoolToRun.getBestPosition();
					feasibilitywrnfp = schoolToRun.getBestFeasibility();
					meanFeasibility+=feasibilitywrnfp[1];
