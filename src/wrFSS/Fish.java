...
public class Fish {
	...
	public void calculateIFeasibility(int iteration){
		...
		if(leader!=null){
			...
			if(Double.compare(fishLastFeasibility[1],fishFeasibility[1])>0){
				for(int i=0;i<IToUpdate.length;i++){
					IToUpdate[i]=(fishPosition[i]-fishLastPosition[i])*(fishLastFeasibility[1]-fishFeasibility[1]);
				}
	...
	public void feedFishFitness(double maxDeltaf) {
		
		updateWeight(weight+(lastFitness-fitness)/maxDeltaf);
		
	}
	
	public void feedFishFeasibility(double maxDeltaFi) {
		
		updateWeight(weight+(lastFeasibility[1]-feasibility[1])/maxDeltaFi);
